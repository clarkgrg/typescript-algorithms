...
export class Token {
  ...
  public isProcedure(): boolean {
    return this.type === eTokens.PROCEDURE;
  }
...
export class Lexer {
  ...
  private RESERVED_WORDS: Record<string, Token> = {
    ...
    PROCEDURE: new Token(eTokens.PROCEDURE, 'PROCEDURE'),
...
/**
 * ProcedureDecl represent a procedure declaration
 */

class ProcedureDecl extends AST {
  public proc_name: string;
  public block_node: AST;
  constructor(proc_name: string, block_node: AST) {
    super();
  ...
  public showType() {
    console.log(`ProcedureDecl ${this.proc_name}`);
    this.block_node.showType();
  }
}
/**
...
export class Parser {
  ...
  private declarations(): AST[] {
    ...
    while (this.current_token?.isProcedure()) {
      this.eat(eTokens.PROCEDURE);
      const proc_name = <string>this.current_token.value;
      this.eat(eTokens.ID);
      this.eat(eTokens.SEMI);
      const block_node = this.block();
...
abstract class NodeVisitor {
  public visit(node: AST): any {
    ...
    } else if (node.getName() === 'ProcedureDecl') {
  ...
  protected abstract visit_ProcedureDecl(node: AST): void;
...
export class SymbolTableBuilder extends NodeVisitor {
  ...
  protected visit_ProcedureDecl(node: ProcedureDecl) {}
...
export class Interpreter extends NodeVisitor {
  ...
  protected visit_ProcedureDecl(node: ProcedureDecl): void {}
